...
mod test_v1_encode_decode {
    ...
    pub fn test_echo_servo_output_raw() {
        ...
        mavlink::write_v2_msg(
            &mut v,
            crate::test_shared::COMMON_MSG_HEADER,
            &mavlink::dialects::common::MavMessage::SERVO_OUTPUT_RAW(send_msg),
        )
        .expect("Failed to write message");
    ...
    pub fn test_extensions_v1() {
        use mavlink::dialects::common::COMMAND_ACK_DATA;
        ...
        let ack_command = COMMAND_ACK_DATA {
            ...
            result: mavlink::dialects::common::MavResult::MAV_RESULT_TEMPORARILY_REJECTED,
            progress: 2,
            result_param2: 3,
            target_system: 4,
            target_component: 5,
        };
        let ack_msg_data = mavlink::dialects::common::MavMessage::COMMAND_ACK(ack_command);
        let mut buf = vec![];
        mavlink::write_v1_msg(
            &mut buf,
            crate::test_shared::COMMON_MSG_HEADER,
        ...
        if let mavlink::dialects::common::MavMessage::COMMAND_ACK(read_ack_command) = read_msg {
            ...
            assert_eq!(
                ...
                mavlink::dialects::common::MavCmd::MAV_CMD_NAV_WAYPOINT
            );
            assert_eq!(
                read_ack_command.result,
                mavlink::dialects::common::MavResult::MAV_RESULT_TEMPORARILY_REJECTED
            );
    ...
    pub fn test_overflowing_msg_id() {
        ...
        let msg_data = mavlink::dialects::common::MavMessage::SETUP_SIGNING(
            mavlink::dialects::common::SETUP_SIGNING_DATA::default(),
        );
        let mut buf = vec![];
        assert!(
            matches!(
